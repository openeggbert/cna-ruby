...
- real native Game lifecycle and GameTime callbacks, including `Game.Tick` as a first-class frame step distinct from `Game.RunOneFrame`, and `Game.IsActive` as the exact `isActive && !(GamerServicesDispatcher.IsInitialized && Guide.IsVisible)` expression over three canonical CNA routes
