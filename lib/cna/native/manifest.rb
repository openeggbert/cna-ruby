...
module CNA
  module Native
    ...
    module Manifest
      ...
      def signature(symbol, return_type, arguments, ownership:, result_lifetime: "thread-local until next failing call", callback_abi: nil, since: nil)
        ...
        arguments.each_with_index do |value, index|
          ...
          value_aggregates[index] = {
            ...
            register_class: value.fetch(:aggregate_class, "INTEGER")
          }
        end
        Signature.new(
          symbol: symbol,
      ...
      # A struct passed by value whose eightbytes the classification puts in **SSE** registers
      # rather than integer ones: `CNA_Vector3` is three floats, so its first eightbyte holds two
      # of them and its second holds the third, and both travel in `xmm0`/`xmm1`.
      #
      # No filler is needed and none would help: the integer and SSE register files are assigned
      # independently, so a handle in `rdi` and two SSE eightbytes in `xmm0`/`xmm1` is exactly what
      # declaring `(uint64, double, double)` produces. A `double`-typed Fiddle argument is loaded
      # into the next XMM register bit for bit, which is why the eightbytes are packed and
      # reinterpreted rather than converted.
      #
      # **Measured, like the other two expansions.** `cna_basic_effect_set_diffuse_color`
      # disassembles to `mov %rdi,…` for the handle, `movq %xmm0,%rax` for the first eightbyte and
      # `movaps %xmm1,%xmm0; movss …` for the second; and a `Vector3` written through this
      # expansion reads back exactly through the matching getter.
      def by_value_sse(c, eightbytes:)
        Array.new(eightbytes) do |index|
          { c: "double", fiddle: F64 }.merge(
            value_aggregate: c, aggregate_start: index.zero?,
            aggregate_members: eightbytes, aggregate_class: "SSE"
          )
        end
      end
      ...
      FUNCTIONS = [
        ...
        signature("cna_graphics_device_draw_user_indexed_primitives", T[:result],
                  ...
                  ownership: "borrows device; reads the caller's arrays for the duration of the call"),
        # The five stock effects, and the three interface contracts they share. `BasicEffect`'s
        ...
        # no filler is needed because the integer and SSE register files are assigned independently.
        # `CNA_Matrix` is 64 bytes, which is MEMORY class like `CNA_Viewport`, so `by_value_memory`
        # expands it into five fillers and eight stack eightbytes. Both were measured from the
        # disassembly and both round-trip exactly through their own getters.
        signature("cna_basic_effect_create", T[:result], [T[:handle], pointer("CNA_EffectHandle")], ownership: "borrows device; returns an OWNED effect"),
        signature("cna_basic_effect_get_alpha", T[:result], [handle("CNA_EffectHandle"), pointer("float")], ownership: "borrows effect; caller output"),
        signature("cna_basic_effect_set_alpha", T[:result], [handle("CNA_EffectHandle"), T[:float]], ownership: "borrows effect"),
        signature("cna_basic_effect_get_specular_power", T[:result], [handle("CNA_EffectHandle"), pointer("float")], ownership: "borrows effect; caller output"),
        signature("cna_basic_effect_set_specular_power", T[:result], [handle("CNA_EffectHandle"), T[:float]], ownership: "borrows effect"),
        signature("cna_basic_effect_get_diffuse_color", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Vector3")], ownership: "borrows effect; caller output"),
        signature("cna_basic_effect_set_diffuse_color", T[:result], [handle("CNA_EffectHandle"), *by_value_sse("CNA_Vector3", eightbytes: 2)], ownership: "borrows effect"),
        signature("cna_basic_effect_get_emissive_color", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Vector3")], ownership: "borrows effect; caller output"),
        signature("cna_basic_effect_set_emissive_color", T[:result], [handle("CNA_EffectHandle"), *by_value_sse("CNA_Vector3", eightbytes: 2)], ownership: "borrows effect"),
        signature("cna_basic_effect_get_specular_color", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Vector3")], ownership: "borrows effect; caller output"),
        signature("cna_basic_effect_set_specular_color", T[:result], [handle("CNA_EffectHandle"), *by_value_sse("CNA_Vector3", eightbytes: 2)], ownership: "borrows effect"),
        signature("cna_basic_effect_get_prefer_per_pixel_lighting", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Bool")], ownership: "borrows effect; caller output"),
        signature("cna_basic_effect_set_prefer_per_pixel_lighting", T[:result], [handle("CNA_EffectHandle"), T[:bool]], ownership: "borrows effect"),
        signature("cna_basic_effect_get_texture_enabled", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Bool")], ownership: "borrows effect; caller output"),
        signature("cna_basic_effect_set_texture_enabled", T[:result], [handle("CNA_EffectHandle"), T[:bool]], ownership: "borrows effect"),
        signature("cna_basic_effect_get_vertex_color_enabled", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Bool")], ownership: "borrows effect; caller output"),
        signature("cna_basic_effect_set_vertex_color_enabled", T[:result], [handle("CNA_EffectHandle"), T[:bool]], ownership: "borrows effect"),
        signature("cna_basic_effect_get_texture", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Bool"), pointer("CNA_Handle")], ownership: "borrows effect; caller output; the texture stays the caller's"),
        signature("cna_basic_effect_set_texture", T[:result], [handle("CNA_EffectHandle"), T[:handle]], ownership: "borrows effect; retains the texture, or clears with the invalid handle"),
        signature("cna_effect_matrices_get_world", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Matrix")], ownership: "borrows effect; caller output"),
        signature("cna_effect_matrices_set_world", T[:result], [handle("CNA_EffectHandle"), *by_value_memory("CNA_Matrix", eightbytes: 8, preceding_integer_arguments: 1)], ownership: "borrows effect"),
        signature("cna_effect_matrices_get_view", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Matrix")], ownership: "borrows effect; caller output"),
        signature("cna_effect_matrices_set_view", T[:result], [handle("CNA_EffectHandle"), *by_value_memory("CNA_Matrix", eightbytes: 8, preceding_integer_arguments: 1)], ownership: "borrows effect"),
        signature("cna_effect_matrices_get_projection", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Matrix")], ownership: "borrows effect; caller output"),
        signature("cna_effect_matrices_set_projection", T[:result], [handle("CNA_EffectHandle"), *by_value_memory("CNA_Matrix", eightbytes: 8, preceding_integer_arguments: 1)], ownership: "borrows effect"),
        signature("cna_effect_lights_get_enabled", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Bool")], ownership: "borrows effect; caller output"),
        signature("cna_effect_lights_set_enabled", T[:result], [handle("CNA_EffectHandle"), T[:bool]], ownership: "borrows effect"),
        signature("cna_effect_lights_get_ambient_color", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Vector3")], ownership: "borrows effect; caller output"),
        signature("cna_effect_lights_set_ambient_color", T[:result], [handle("CNA_EffectHandle"), *by_value_sse("CNA_Vector3", eightbytes: 2)], ownership: "borrows effect"),
        signature("cna_effect_lights_enable_default", T[:result], [handle("CNA_EffectHandle")], ownership: "borrows effect"),
        signature("cna_effect_lights_get_directional_light", T[:result], [handle("CNA_EffectHandle"), T[:u32], pointer("CNA_DirectionalLightHandle")], ownership: "borrows effect; returns an OWNED member view, released with cna_directional_light_destroy"),
        signature("cna_effect_fog_get_enabled", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Bool")], ownership: "borrows effect; caller output"),
        signature("cna_effect_fog_set_enabled", T[:result], [handle("CNA_EffectHandle"), T[:bool]], ownership: "borrows effect"),
        signature("cna_effect_fog_get_start", T[:result], [handle("CNA_EffectHandle"), pointer("float")], ownership: "borrows effect; caller output"),
        signature("cna_effect_fog_set_start", T[:result], [handle("CNA_EffectHandle"), T[:float]], ownership: "borrows effect"),
        signature("cna_effect_fog_get_end", T[:result], [handle("CNA_EffectHandle"), pointer("float")], ownership: "borrows effect; caller output"),
        signature("cna_effect_fog_set_end", T[:result], [handle("CNA_EffectHandle"), T[:float]], ownership: "borrows effect"),
        signature("cna_effect_fog_get_color", T[:result], [handle("CNA_EffectHandle"), pointer("CNA_Vector3")], ownership: "borrows effect; caller output"),
        signature("cna_effect_fog_set_color", T[:result], [handle("CNA_EffectHandle"), *by_value_sse("CNA_Vector3", eightbytes: 2)], ownership: "borrows effect"),
        signature("cna_directional_light_get_enabled", T[:result], [handle("CNA_DirectionalLightHandle"), pointer("CNA_Bool")], ownership: "borrows light; caller output"),
        signature("cna_directional_light_set_enabled", T[:result], [handle("CNA_DirectionalLightHandle"), T[:bool]], ownership: "borrows light"),
        signature("cna_directional_light_get_direction", T[:result], [handle("CNA_DirectionalLightHandle"), pointer("CNA_Vector3")], ownership: "borrows light; caller output"),
        signature("cna_directional_light_set_direction", T[:result], [handle("CNA_DirectionalLightHandle"), *by_value_sse("CNA_Vector3", eightbytes: 2)], ownership: "borrows light"),
        signature("cna_directional_light_get_diffuse_color", T[:result], [handle("CNA_DirectionalLightHandle"), pointer("CNA_Vector3")], ownership: "borrows light; caller output"),
        signature("cna_directional_light_set_diffuse_color", T[:result], [handle("CNA_DirectionalLightHandle"), *by_value_sse("CNA_Vector3", eightbytes: 2)], ownership: "borrows light"),
        ...
        # The Media namespace: seventeen types over one library. Every route here has a production
        # call site — the family's `_ext` routes with no XNA member behind them stay unbound, and so
        # does the `get_type_name_size`/`copy_type_name` pair every handle carries, which nothing in
        # the selected surface reads.
        #
        # `MediaPlayer` is a **static** XNA class and CNA's routes take the Game handle rather than a
        # player handle, which is the same shape: one player per process, reached through the game.
        # Its two events are process-global subscriptions, `_ext` because XNA's own raisers are
        # private.
        #
        # MEASURED: `Song.Album`, `Song.Artist` and `Song.Genre` have **no CNA route at all** —
        # every other member of every one of the seventeen does. Those three are the family's only
        # blocked members and `Song` is the only partial type it leaves.
        signature("cna_media_library_create", T[:result], [T[:handle], pointer("CNA_MediaLibraryHandle")], ownership: "returns an OWNED handle"),
        signature("cna_media_library_create_from_source", T[:result], [T[:handle], T[:u32], pointer("CNA_MediaLibraryHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_is_disposed", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_dispose", T[:result], [handle("CNA_MediaLibraryHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_media_library_destroy", T[:result], [handle("CNA_MediaLibraryHandle")], ownership: "releases one owned handle"),
        signature("cna_media_library_get_media_source_type", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("CNA_MediaSourceType")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_media_source_name_size", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_copy_media_source_name", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("char"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_songs", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("CNA_SongCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_albums", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("CNA_AlbumCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_artists", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("CNA_ArtistCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_genres", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("CNA_GenreCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_playlists", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("CNA_PlaylistCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_pictures", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("CNA_PictureCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_saved_pictures", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("CNA_PictureCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_root_picture_album", T[:result], [handle("CNA_MediaLibraryHandle"), pointer("CNA_PictureAlbumHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_get_picture_from_token", T[:result], [handle("CNA_MediaLibraryHandle"), *by_value("CNA_StringView", pointer("char", const: true), T[:u64]), pointer("CNA_PictureHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_media_library_save_picture", T[:result], [handle("CNA_MediaLibraryHandle"), *by_value("CNA_StringView", pointer("char", const: true), T[:u64]), pointer("uint8_t", const: true), T[:u64], pointer("CNA_PictureHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_get_game_has_control", T[:result], [T[:handle], pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_get_is_muted", T[:result], [T[:handle], pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_set_is_muted", T[:result], [T[:handle], T[:bool]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_get_is_repeating", T[:result], [T[:handle], pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_set_is_repeating", T[:result], [T[:handle], T[:bool]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_get_is_shuffled", T[:result], [T[:handle], pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_set_is_shuffled", T[:result], [T[:handle], T[:bool]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_get_play_position_ticks", T[:result], [T[:handle], pointer("int64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_get_state", T[:result], [T[:handle], pointer("CNA_MediaState")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_get_volume", T[:result], [T[:handle], pointer("float")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_set_volume", T[:result], [T[:handle], T[:float]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_get_is_visualization_enabled", T[:result], [T[:handle], pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_set_is_visualization_enabled", T[:result], [T[:handle], T[:bool]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_get_visualization_data", T[:result], [T[:handle], pointer("CNA_VisualizationData")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_get_queue", T[:result], [T[:handle], pointer("CNA_MediaQueueHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_play_song", T[:result], [T[:handle], handle("CNA_SongHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_play_songs", T[:result], [T[:handle], handle("CNA_SongCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_play_songs_from", T[:result], [T[:handle], handle("CNA_SongCollectionHandle"), T[:i32]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_move_next", T[:result], [T[:handle]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_move_previous", T[:result], [T[:handle]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_pause", T[:result], [T[:handle]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_resume", T[:result], [T[:handle]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_stop", T[:result], [T[:handle]], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_subscribe_active_song_changed_ext", T[:result], [callback_pointer("CNA_MediaPlayerEventCallback"), pointer("void"), pointer("CNA_MediaPlayerEventRegistrationHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_subscribe_media_state_changed_ext", T[:result], [callback_pointer("CNA_MediaPlayerEventCallback"), pointer("void"), pointer("CNA_MediaPlayerEventRegistrationHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_player_unsubscribe_ext", T[:result], [handle("CNA_MediaPlayerEventRegistrationHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_queue_get_count", T[:result], [handle("CNA_MediaQueueHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_media_queue_get_active_song_index", T[:result], [handle("CNA_MediaQueueHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_media_queue_set_active_song_index", T[:result], [handle("CNA_MediaQueueHandle"), T[:i32]], ownership: "borrows the handle"),
        signature("cna_media_queue_get_active_song", T[:result], [handle("CNA_MediaQueueHandle"), pointer("CNA_SongHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_media_queue_get_at", T[:result], [handle("CNA_MediaQueueHandle"), T[:i32], pointer("CNA_SongHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_media_queue_destroy", T[:result], [handle("CNA_MediaQueueHandle")], ownership: "releases one owned handle"),
        signature("cna_song_create_from_uri", T[:result], [T[:handle], *by_value("CNA_StringView", pointer("char", const: true), T[:u64]), *by_value("CNA_StringView", pointer("char", const: true), T[:u64]), pointer("CNA_SongHandle")], ownership: "returns an OWNED handle"),
        signature("cna_song_get_name_size", T[:result], [handle("CNA_SongHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_song_copy_name", T[:result], [handle("CNA_SongHandle"), pointer("char"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_song_get_duration", T[:result], [handle("CNA_SongHandle"), pointer("int64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_song_get_is_protected", T[:result], [handle("CNA_SongHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_song_get_is_rated", T[:result], [handle("CNA_SongHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        ...
        signature("cna_song_equals", T[:result], [handle("CNA_SongHandle"), handle("CNA_SongHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_song_get_hash_code", T[:result], [handle("CNA_SongHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_song_collection_get_at", T[:result], [handle("CNA_SongCollectionHandle"), T[:i32], pointer("CNA_SongHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_song_collection_get_count", T[:result], [handle("CNA_SongCollectionHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_song_collection_get_is_disposed", T[:result], [handle("CNA_SongCollectionHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_song_collection_dispose", T[:result], [handle("CNA_SongCollectionHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_song_collection_destroy", T[:result], [handle("CNA_SongCollectionHandle")], ownership: "releases one owned handle"),
        signature("cna_album_get_name_size", T[:result], [handle("CNA_AlbumHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_album_copy_name", T[:result], [handle("CNA_AlbumHandle"), pointer("char"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_album_get_is_disposed", T[:result], [handle("CNA_AlbumHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_album_dispose", T[:result], [handle("CNA_AlbumHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_album_destroy", T[:result], [handle("CNA_AlbumHandle")], ownership: "releases one owned handle"),
        signature("cna_album_get_hash_code", T[:result], [handle("CNA_AlbumHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_album_get_songs", T[:result], [handle("CNA_AlbumHandle"), pointer("CNA_SongCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_album_equals", T[:result], [handle("CNA_AlbumHandle"), handle("CNA_AlbumHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_album_get_artist", T[:result], [handle("CNA_AlbumHandle"), pointer("CNA_ArtistHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_album_get_genre", T[:result], [handle("CNA_AlbumHandle"), pointer("CNA_GenreHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_album_get_duration", T[:result], [handle("CNA_AlbumHandle"), pointer("int64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_album_get_has_art", T[:result], [handle("CNA_AlbumHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_album_get_art_size", T[:result], [handle("CNA_AlbumHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_album_copy_art", T[:result], [handle("CNA_AlbumHandle"), pointer("uint8_t"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_album_get_thumbnail_size", T[:result], [handle("CNA_AlbumHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_album_copy_thumbnail", T[:result], [handle("CNA_AlbumHandle"), pointer("uint8_t"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_album_collection_get_count", T[:result], [handle("CNA_AlbumCollectionHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_album_collection_get_at", T[:result], [handle("CNA_AlbumCollectionHandle"), T[:i32], pointer("CNA_AlbumHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_album_collection_get_is_disposed", T[:result], [handle("CNA_AlbumCollectionHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_album_collection_dispose", T[:result], [handle("CNA_AlbumCollectionHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_album_collection_destroy", T[:result], [handle("CNA_AlbumCollectionHandle")], ownership: "releases one owned handle"),
        signature("cna_artist_get_name_size", T[:result], [handle("CNA_ArtistHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_artist_copy_name", T[:result], [handle("CNA_ArtistHandle"), pointer("char"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_artist_get_is_disposed", T[:result], [handle("CNA_ArtistHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_artist_dispose", T[:result], [handle("CNA_ArtistHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_artist_destroy", T[:result], [handle("CNA_ArtistHandle")], ownership: "releases one owned handle"),
        signature("cna_artist_get_hash_code", T[:result], [handle("CNA_ArtistHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_artist_get_songs", T[:result], [handle("CNA_ArtistHandle"), pointer("CNA_SongCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_artist_get_albums", T[:result], [handle("CNA_ArtistHandle"), pointer("CNA_AlbumCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_artist_equals", T[:result], [handle("CNA_ArtistHandle"), handle("CNA_ArtistHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_artist_collection_get_count", T[:result], [handle("CNA_ArtistCollectionHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_artist_collection_get_at", T[:result], [handle("CNA_ArtistCollectionHandle"), T[:i32], pointer("CNA_ArtistHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_artist_collection_get_is_disposed", T[:result], [handle("CNA_ArtistCollectionHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_artist_collection_dispose", T[:result], [handle("CNA_ArtistCollectionHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_artist_collection_destroy", T[:result], [handle("CNA_ArtistCollectionHandle")], ownership: "releases one owned handle"),
        signature("cna_genre_get_name_size", T[:result], [handle("CNA_GenreHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_genre_copy_name", T[:result], [handle("CNA_GenreHandle"), pointer("char"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_genre_get_is_disposed", T[:result], [handle("CNA_GenreHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_genre_dispose", T[:result], [handle("CNA_GenreHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_genre_destroy", T[:result], [handle("CNA_GenreHandle")], ownership: "releases one owned handle"),
        signature("cna_genre_get_hash_code", T[:result], [handle("CNA_GenreHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_genre_get_songs", T[:result], [handle("CNA_GenreHandle"), pointer("CNA_SongCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_genre_get_albums", T[:result], [handle("CNA_GenreHandle"), pointer("CNA_AlbumCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_genre_equals", T[:result], [handle("CNA_GenreHandle"), handle("CNA_GenreHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_genre_collection_get_count", T[:result], [handle("CNA_GenreCollectionHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_genre_collection_get_at", T[:result], [handle("CNA_GenreCollectionHandle"), T[:i32], pointer("CNA_GenreHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_genre_collection_get_is_disposed", T[:result], [handle("CNA_GenreCollectionHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_genre_collection_dispose", T[:result], [handle("CNA_GenreCollectionHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_genre_collection_destroy", T[:result], [handle("CNA_GenreCollectionHandle")], ownership: "releases one owned handle"),
        signature("cna_playlist_get_name_size", T[:result], [handle("CNA_PlaylistHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_playlist_copy_name", T[:result], [handle("CNA_PlaylistHandle"), pointer("char"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_playlist_get_is_disposed", T[:result], [handle("CNA_PlaylistHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_playlist_dispose", T[:result], [handle("CNA_PlaylistHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_playlist_destroy", T[:result], [handle("CNA_PlaylistHandle")], ownership: "releases one owned handle"),
        signature("cna_playlist_get_hash_code", T[:result], [handle("CNA_PlaylistHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_playlist_get_songs", T[:result], [handle("CNA_PlaylistHandle"), pointer("CNA_SongCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_playlist_get_duration", T[:result], [handle("CNA_PlaylistHandle"), pointer("int64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_playlist_equals", T[:result], [handle("CNA_PlaylistHandle"), handle("CNA_PlaylistHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_playlist_collection_get_count", T[:result], [handle("CNA_PlaylistCollectionHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_playlist_collection_get_at", T[:result], [handle("CNA_PlaylistCollectionHandle"), T[:i32], pointer("CNA_PlaylistHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_playlist_collection_get_is_disposed", T[:result], [handle("CNA_PlaylistCollectionHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_playlist_collection_dispose", T[:result], [handle("CNA_PlaylistCollectionHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_playlist_collection_destroy", T[:result], [handle("CNA_PlaylistCollectionHandle")], ownership: "releases one owned handle"),
        signature("cna_picture_get_name_size", T[:result], [handle("CNA_PictureHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_copy_name", T[:result], [handle("CNA_PictureHandle"), pointer("char"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_get_album", T[:result], [handle("CNA_PictureHandle"), pointer("CNA_PictureAlbumHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_get_date_unix_ticks", T[:result], [handle("CNA_PictureHandle"), pointer("int64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_get_width", T[:result], [handle("CNA_PictureHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_get_height", T[:result], [handle("CNA_PictureHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_get_image_size", T[:result], [handle("CNA_PictureHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_copy_image", T[:result], [handle("CNA_PictureHandle"), pointer("uint8_t"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_get_thumbnail_size", T[:result], [handle("CNA_PictureHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_copy_thumbnail", T[:result], [handle("CNA_PictureHandle"), pointer("uint8_t"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_get_is_disposed", T[:result], [handle("CNA_PictureHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_dispose", T[:result], [handle("CNA_PictureHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_picture_destroy", T[:result], [handle("CNA_PictureHandle")], ownership: "releases one owned handle"),
        signature("cna_picture_equals", T[:result], [handle("CNA_PictureHandle"), handle("CNA_PictureHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_get_hash_code", T[:result], [handle("CNA_PictureHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_get_name_size", T[:result], [handle("CNA_PictureAlbumHandle"), pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_copy_name", T[:result], [handle("CNA_PictureAlbumHandle"), pointer("char"), T[:u64], pointer("uint64_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_get_parent", T[:result], [handle("CNA_PictureAlbumHandle"), pointer("CNA_PictureAlbumHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_get_albums", T[:result], [handle("CNA_PictureAlbumHandle"), pointer("CNA_PictureAlbumCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_get_pictures", T[:result], [handle("CNA_PictureAlbumHandle"), pointer("CNA_PictureCollectionHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_get_is_disposed", T[:result], [handle("CNA_PictureAlbumHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_dispose", T[:result], [handle("CNA_PictureAlbumHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_picture_album_destroy", T[:result], [handle("CNA_PictureAlbumHandle")], ownership: "releases one owned handle"),
        signature("cna_picture_album_equals", T[:result], [handle("CNA_PictureAlbumHandle"), handle("CNA_PictureAlbumHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_get_hash_code", T[:result], [handle("CNA_PictureAlbumHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_collection_get_count", T[:result], [handle("CNA_PictureCollectionHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_collection_get_at", T[:result], [handle("CNA_PictureCollectionHandle"), T[:i32], pointer("CNA_PictureHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_collection_get_is_disposed", T[:result], [handle("CNA_PictureCollectionHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_collection_dispose", T[:result], [handle("CNA_PictureCollectionHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_picture_collection_destroy", T[:result], [handle("CNA_PictureCollectionHandle")], ownership: "releases one owned handle"),
        signature("cna_picture_album_collection_get_count", T[:result], [handle("CNA_PictureAlbumCollectionHandle"), pointer("int32_t")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_collection_get_at", T[:result], [handle("CNA_PictureAlbumCollectionHandle"), T[:i32], pointer("CNA_PictureAlbumHandle")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_collection_get_is_disposed", T[:result], [handle("CNA_PictureAlbumCollectionHandle"), pointer("CNA_Bool")], ownership: "borrows the handle; caller output"),
        signature("cna_picture_album_collection_dispose", T[:result], [handle("CNA_PictureAlbumCollectionHandle")], ownership: "borrows the handle; performs XNA's Dispose"),
        signature("cna_picture_album_collection_destroy", T[:result], [handle("CNA_PictureAlbumCollectionHandle")], ownership: "releases one owned handle")
      ].freeze
