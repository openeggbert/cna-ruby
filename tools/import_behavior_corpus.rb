...
device_service_path = File.expand_path("../behavior/xna40-graphics-device-service-values.json", __dir__)
device_service = JSON.parse(File.read(device_service_path))
unless device_service["category"] == "MIXED_WITH_OBSERVATION_PROVENANCE"
  abort "Milestone 40 graphics device service evidence lacks observation-level provenance"
end
observations.concat(device_service.fetch("observations"))
game_events_path = File.expand_path("../behavior/xna40-game-event-values.json", __dir__)
game_events = JSON.parse(File.read(game_events_path))
unless game_events["category"] == "MIXED_WITH_OBSERVATION_PROVENANCE"
  abort "Milestone 41 Game event evidence lacks observation-level provenance"
end
observations.concat(game_events.fetch("observations"))
game_timing_path = File.expand_path("../behavior/xna40-game-timing-values.json", __dir__)
game_timing = JSON.parse(File.read(game_timing_path))
unless game_timing["category"] == "MIXED_WITH_OBSERVATION_PROVENANCE"
...
game_is_active_path = File.expand_path("../behavior/xna40-game-is-active-values.json", __dir__)
game_is_active = JSON.parse(File.read(game_is_active_path))
unless game_is_active["category"] == "MIXED_WITH_OBSERVATION_PROVENANCE"
  abort "Milestone 45 Game.IsActive evidence lacks observation-level provenance"
end
observations.concat(game_is_active.fetch("observations"))
dictionary_path = File.expand_path("../behavior/xna40-dictionary-values.json", __dir__)
dictionary = JSON.parse(File.read(dictionary_path))
unless dictionary["category"] == "MIXED_WITH_OBSERVATION_PROVENANCE"
  abort "Milestone 46 Dictionary evidence lacks observation-level provenance"
end
observations.concat(dictionary.fetch("observations"))
game_disposal_path = File.expand_path("../behavior/xna40-game-disposal-values.json", __dir__)
game_disposal = JSON.parse(File.read(game_disposal_path))
unless game_disposal["category"] == "MIXED_WITH_OBSERVATION_PROVENANCE"
  abort "Milestone 47 Game disposal evidence lacks observation-level provenance"
end
observations.concat(game_disposal.fetch("observations"))
game_window_path = File.expand_path("../behavior/xna40-game-window-values.json", __dir__)
game_window = JSON.parse(File.read(game_window_path))
unless game_window["category"] == "MIXED_WITH_OBSERVATION_PROVENANCE"
  abort "Milestone 48 GameWindow evidence lacks observation-level provenance"
end
observations.concat(game_window.fetch("observations"))
serialization_path = File.expand_path("../behavior/xna40-serialization-values.json", __dir__)
serialization = JSON.parse(File.read(serialization_path))
unless serialization["category"] == "MIXED_WITH_OBSERVATION_PROVENANCE"
...
visualization_path = File.expand_path("../behavior/xna40-visualization-data-values.json", __dir__)
visualization = JSON.parse(File.read(visualization_path))
unless visualization["category"] == "MIXED_WITH_OBSERVATION_PROVENANCE"
  abort "Milestone 51 VisualizationData evidence lacks observation-level provenance"
end
observations.concat(visualization.fetch("observations"))
media_video_path = File.expand_path("../behavior/xna40-media-video-values.json", __dir__)
media_video = JSON.parse(File.read(media_video_path))
unless media_video["category"] == "MIXED_WITH_OBSERVATION_PROVENANCE"
  abort "Milestone 52 Media.Video evidence lacks observation-level provenance"
end
observations.concat(media_video.fetch("observations"))
...
result = source_corpus.merge(
  ...
  "milestone45SourceAssemblySha256" => game_is_active.fetch("sourceAssemblySha256"),
  "milestone45SourceGamerServicesAssemblySha256" => game_is_active.fetch("sourceGamerServicesAssemblySha256"),
  "milestone46SourceAssemblySha256" => dictionary.fetch("sourceAssemblySha256"),
  "milestone46SourceBclAssemblySha256" => dictionary.fetch("sourceBclAssemblySha256"),
  "milestone47SourceAssemblySha256" => game_disposal.fetch("sourceAssemblySha256"),
  "milestone48SourceAssemblySha256" => game_window.fetch("sourceAssemblySha256"),
  ...
  "milestone51SourceAssemblySha256" => visualization.fetch("sourceAssemblySha256"),
  "milestone52SourceAssemblySha256" => media_video.fetch("sourceAssemblySha256"),
